...
-- ── MIGRATION: Add chat & messaging columns for existing DBs ──
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS type VARCHAR(20) NOT NULL DEFAULT 'USED';
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS entity_type VARCHAR(30);
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS entity_id BIGINT;
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS title VARCHAR(255);
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS status VARCHAR(20) NOT NULL DEFAULT 'OPEN';
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS last_message_at TIMESTAMP;
ALTER TABLE conversation_members ADD COLUMN IF NOT EXISTS role VARCHAR(20);
...
-- Enable Realtime for messages table (requires Supabase project setting)
-- Run in Supabase SQL Editor: ALTER PUBLICATION supabase_realtime ADD TABLE messages;
