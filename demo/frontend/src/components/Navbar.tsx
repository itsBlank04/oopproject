              ...
              style={{ width: searchOpen ? '260px' : '0px', opacity: searchOpen ? 1 : 0 }}>
                <div className="flex w-[260px] flex-shrink-0 items-center gap-2">
                  <div className="relative flex-1">
                    <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8c7564]" />
                    <input
                      ref={searchInputRef}
                      type="text"
                      value={searchQuery}
                      ...
                      placeholder="Search..."
                      className="w-full rounded-full border border-[#e4d6c8] bg-[#f9f5f0] py-1.5 pl-9 pr-3 text-sm text-[#221b16] placeholder:text-[#8c7564] outline-none transition-colors focus:border-[#8c7564] focus:bg-white"
                    />
                  </div>
                  <button
          ...
          {user ? (
            <>
              <div className="hidden items-center gap-3 md:flex">
                ...
                <Link to="/wishlist" className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-white/60 bg-white/40 text-[#6c5b4f] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.06)] backdrop-blur-md transition-all duration-300 hover:border-[#e4d6c8] hover:bg-white/70 hover:text-[#221b16]">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-[18px] w-[18px]">
              ...
              <div className="relative" ref={menuRef}>
                <button onClick={() => { setMenuOpen(!menuOpen); }} className="flex items-center gap-2 rounded-xl border border-white/60 bg-white/40 px-3 py-1.5 text-sm text-[#221b16] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.06)] backdrop-blur-md transition-all duration-300 hover:border-[#e4d6c8] hover:bg-white/70">
                {user.avatarUrl ? (
              ...
              {menuOpen && (
                ...
                {subscribedRoles.length > 0 && (
                  <div className="border-t border-[#e4d6c8]/40 mt-1 pt-1 px-1">
                    <p className="px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8c7564]">Switch Mode</p>
                    {subscribedRoles.includes('vendor') && (
                      ...
                      >
                        ...
                        <span className={`relative inline-flex h-5 w-9 items-center rounded-full transition-all duration-300 ${activeRole === 'vendor' ? 'bg-emerald-500' : 'bg-[#e4d6c8]'}`}>
            ...
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/60 bg-white/40 text-[#6c5b4f] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.06)] backdrop-blur-md transition-all duration-300 hover:border-[#e4d6c8] hover:bg-white/70 hover:text-[#221b16]">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-[18px] w-[18px]">
