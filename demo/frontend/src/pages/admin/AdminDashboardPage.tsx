...
export default function AdminDashboardPage() {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-[#f9f5f0]"><div className="font-[Fraunces] text-lg text-[#6c5b4f]">Loading admin…</div></div>

  if (!user || !hasRole('ADMIN')) return (
    <div className="flex min-h-screen items-center justify-center bg-[#f9f5f0] p-8">
      <div className="max-w-lg rounded-2xl border border-[#e4d6c8] bg-white p-10 text-center shadow-sm">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#f9f5f0] text-2xl">⚜</div>
        <p className="text-xs font-bold uppercase tracking-[0.25em] text-[#c4956a]">Restricted</p>
        <h1 className="mt-2 font-[Fraunces] text-3xl text-[#221b16]">Admin Access Required</h1>
        <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-[#6c5b4f]">Sign in with the primary admin account to access the governance panel.</p>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#f9f5f0] text-[#221b16]">
      {/* HEADER */}
      <header className="border-b border-[#e4d6c8] bg-[#221b16] text-[#f9f5f0]">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-6 py-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#c4956a]">AtomDrops Governance</p>
            <h1 className="mt-2 font-[Fraunces] text-4xl">Admin Control Room</h1>
            <p className="mt-2 max-w-xl text-sm leading-6 text-[#e4d6c8]">Users, marketplace, orders, auctions, cases, settings, and audit — all in one command centre.</p>
          </div>
          <div className="flex items-center gap-4 rounded-xl border border-[#6c5b4f] bg-[#3a3028] px-5 py-3 text-sm">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#c4956a] text-xs font-bold text-[#221b16]">A</div>
            <div>
              <p className="text-[#e4d6c8] text-xs">Signed in as</p>
              <p className="font-semibold text-[#f9f5f0]">{user.displayName}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-6 py-6">
        <div className="grid gap-6 lg:grid-cols-[200px_1fr]">
          {/* SIDEBAR */}
          <aside className="h-fit rounded-xl border border-[#e4d6c8] bg-white p-1.5 shadow-sm">
            {tabs.map(item => (
              <button key={item.id} onClick={() => setTab(item.id)}
                className={`flex w-full items-center gap-3 rounded-lg px-3.5 py-2.5 text-left text-sm font-semibold transition-all ${tab === item.id ? 'bg-[#221b16] text-[#f9f5f0] shadow-sm' : 'text-[#6c5b4f] hover:bg-[#f9f5f0]'}`}>
          ...
          <main className="space-y-6">
            {tab === 'overview' && <>
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {metricRows.map(([label, value]) => (
                  <div key={label} className="rounded-xl border border-[#e4d6c8] bg-white p-5 shadow-sm transition hover:shadow-md">
                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#c4956a]">{label}</p>
            ...
            {tab === 'users' && <section className="overflow-hidden rounded-xl border border-[#e4d6c8] bg-white shadow-sm">
              <div className="flex flex-col gap-3 border-b border-[#e4d6c8] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <h2 className="font-[Fraunces] text-xl">User Governance</h2>
                <div className="flex items-center gap-3">
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name or email…" className="w-56 rounded-lg border border-[#e4d6c8] px-3.5 py-2 text-sm outline-none transition focus:border-[#221b16]" />
                  <span className="text-xs font-medium text-[#6c5b4f]">{visibleUserRows.length} of {userRows.length}</span>
                </div>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1000px] text-left text-sm">
                  <thead className="bg-[#f9f5f0] text-[11px] font-bold uppercase tracking-[0.15em] text-[#6c5b4f]">
                  ...
                  <tbody>
                    {visibleUserRows.map(row => {
                      ...
                      return (
                        <tr key={row.id} className="border-t border-[#f9f5f0] transition hover:bg-[#f9f5f0]">
                          <td className="px-5 py-4"><p className="font-semibold">{row.displayName}</p><p className="text-xs text-[#6c5b4f]">{row.email}</p></td>
                          <td className="px-5 py-4"><Pill value={row.status} /></td>
                          <td className="px-5 py-4">
                            {primary ? <span className="text-xs text-[#6c5b4f]">—</span> : (
                              <div className="flex items-center gap-2">
                                ...
                                <input value={redFlagReasons[row.id] ?? ''} onChange={e => setRedFlagReasons(prev => ({ ...prev, [row.id]: e.target.value }))}
                                  placeholder="Reason…" className="w-32 rounded border border-[#e4d6c8] px-2 py-1 text-xs outline-none focus:border-[#221b16]" />
                                <button onClick={() => {
                          ...
                          <td className="px-5 py-4">
                            <div className="flex flex-wrap gap-1.5">
                              {primary ? <span className="rounded-full border border-[#221b16] bg-[#221b16] px-2.5 py-0.5 text-xs font-bold text-[#f9f5f0]">ADMIN</span> :
                                marketplaceRoles.map(role => (
                                  <button key={role} onClick={() => toggleRole(row.id, role, row.roles || [])}
                                    className={`rounded-full border px-2.5 py-0.5 text-xs font-bold transition ${roles.includes(role) ? 'border-[#221b16] bg-[#221b16] text-[#f9f5f0]' : 'border-[#e4d6c8] text-[#6c5b4f] hover:bg-[#f9f5f0]'}`}>{role}</button>
                                ))
                              }
                            </div>
                          </td>
                          <td className="px-5 py-4">
                            <div className="flex flex-wrap gap-1.5">
                              {primary ? <span className="rounded-lg border border-[#e4d6c8] px-3 py-1.5 text-xs font-semibold text-[#6c5b4f]">Primary</span> : <>
              ...
              {userRows.length > 30 && <p className="border-t border-[#e4d6c8] px-5 py-3 text-xs text-[#6c5b4f]">Narrow the search to see users beyond the first 30.</p>}
            </section>}
            ...
            {tab === 'market' && <div className="grid gap-4 xl:grid-cols-3">
              <Panel title="New Products" count={products.data?.length || 0}>
                {(products.data || []).slice(0, 20).map(item => (
                  <div key={item.id} className="flex items-center justify-between gap-3 border-t border-[#e4d6c8] px-5 py-3.5">
            ...
            {tab === 'orders' && <Panel title="Order Oversight" count={orders.data?.length || 0}>
              {(orders.data || []).slice(0, 50).map(item => (
                <div key={item.id} className="flex flex-col gap-2 border-t border-[#e4d6c8] px-5 py-3.5 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <select value={orderOverrides[item.id] ?? item.status} onChange={e => setOrderOverrides(prev => ({ ...prev, [item.id]: e.target.value }))}
                      className="rounded-lg border border-[#e4d6c8] px-2.5 py-1.5 text-xs outline-none focus:border-[#221b16]">
            ...
            {tab === 'auctions' && <Panel title="Auction Moderation" count={auctions.data?.length || 0}>
              {(auctions.data || []).map(item => {
                ...
                return (
                <div key={item.id} className="flex flex-col gap-3 border-t border-[#e4d6c8] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-semibold">{item.title}</p>
                    <p className="mt-0.5 truncate text-xs text-[#6c5b4f]">{item.type} · {item.vendor?.displayName || 'Vendor'} · {item.lots?.length || 0} lot(s)</p>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            {tab === 'settings' && <Panel title="Platform Settings" count={settings.data?.length || 0}>
                {(settings.data || []).map(item => (
                  <div key={item.key} className="grid gap-3 border-t border-[#e4d6c8] px-5 py-4 lg:grid-cols-[1fr_200px_auto] lg:items-center">
                    <div><p className="font-semibold">{item.key}</p><p className="text-xs text-[#6c5b4f]">{item.description || item.dataType}</p></div>
                    <input value={settingDrafts[item.key] ?? item.value} onChange={e => setSettingDrafts(p => ({ ...p, [item.key]: e.target.value }))} className="rounded-lg border border-[#e4d6c8] px-3 py-2 text-sm outline-none transition focus:border-[#221b16]" />
            ...
            {tab === 'notices' && <div className="grid gap-4 xl:grid-cols-[380px_1fr]">
              <div className="rounded-xl border border-[#e4d6c8] bg-white p-5 shadow-sm">
                <h2 className="font-[Fraunces] text-xl">Create Notice</h2>
                <div className="mt-4 space-y-3">
                  <input value={notice.title} onChange={e => setNotice(p => ({ ...p, title: e.target.value }))} placeholder="Title" className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2.5 text-sm outline-none transition focus:border-[#221b16]" />
                  <textarea value={notice.body} onChange={e => setNotice(p => ({ ...p, body: e.target.value }))} placeholder="Body" rows={4} className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2.5 text-sm outline-none transition focus:border-[#221b16]" />
                  <input value={notice.targetRoles} onChange={e => setNotice(p => ({ ...p, targetRoles: e.target.value }))} placeholder="Target roles (comma-separated)" className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2.5 text-sm outline-none transition focus:border-[#221b16]" />
                  <input type="datetime-local" value={notice.startsAt} onChange={e => setNotice(p => ({ ...p, startsAt: e.target.value }))} className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2.5 text-sm outline-none transition focus:border-[#221b16]" />
              ...
              {(upgrades.data || []).map(item => (
                <div key={item.id} className="flex flex-col gap-3 border-t border-[#e4d6c8] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-[#6c5b4f]">{item.user?.email || '—'} · {item.role} · {dateLabel(item.createdAt)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Pill value={item.status} />
                    {item.status === 'ACTIVE' && (
            ...
            {tab === 'plans' && <Panel title="Subscription Plans" count={plans.data?.length || 0}>
              {/* Plan form */}
              <div className="border-b border-[#e4d6c8] p-5 space-y-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6c5b4f] mb-1">Features (one per line)</label>
                  <textarea value={planForm.features || ''} onChange={e => setPlanForm(p => ({ ...p, features: e.target.value }))} rows={3} className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2 text-sm outline-none transition focus:border-[#221b16]" />
                </div>
              ...
              {/* Plan list */}
              {(plans.data || []).map(p => (
                <div key={p.id} className="flex items-center justify-between border-t border-[#e4d6c8] px-5 py-4">
                  <div>
                    <p className="font-semibold">{p.displayName}</p>
                    <p className="text-xs text-[#6c5b4f]">{p.name} · {p.maxShops} shops · {money(p.priceMonthlyBdt)}/mo · {money(p.priceYearlyBdt)}/yr</p>
                  </div>
                  <div className="flex gap-1.5">
            ...
            {tab === 'deals' && <Panel title="Subscription Deals" count={deals.data?.length || 0}>
              ...
              <div className="border-b border-[#e4d6c8] p-5 space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <PlanField label="Type" value={dealForm.dealType || 'DISCOUNT'} onChange={v => setDealForm(p => ({ ...p, dealType: v }))}
                    render={() => <select value={dealForm.dealType || 'DISCOUNT'} onChange={e => setDealForm(p => ({ ...p, dealType: e.target.value }))}
                      className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2.5 text-sm outline-none transition focus:border-[#221b16]"
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6c5b4f] mb-1">Description</label>
                  <textarea value={dealForm.description || ''} onChange={e => setDealForm(p => ({ ...p, description: e.target.value }))} rows={2} className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2 text-sm outline-none transition focus:border-[#221b16]" />
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
              ...
              {/* Deal list */}
              {(deals.data || []).map(d => (
                <div key={d.id} className="flex items-center justify-between border-t border-[#e4d6c8] px-5 py-4">
                  <div>
                    <p className="font-semibold">{d.title}</p>
                    <p className="text-xs text-[#6c5b4f]">{d.dealType} · {money(d.value)} · {d.plan ? d.plan.displayName : 'All plans'} · {d.isActive ? 'Active' : 'Inactive'}</p>
                  </div>
            ...
            {tab === 'verification' && <div className="grid gap-4 xl:grid-cols-2">
              <Panel title="Vendor Verification" count={vendors.data?.length || 0}>
                {(vendors.data || []).map(item => (
                  <div key={item.id} className="flex items-center justify-between border-t border-[#e4d6c8] px-5 py-4">
                    <div className="min-w-0"><p className="truncate font-semibold">{item.shopName}</p><p className="text-xs text-[#6c5b4f]">{item.user?.displayName || 'Vendor'}</p></div>
                    <div className="flex items-center gap-2">
              ...
              <Panel title="Shop Verification Levels" count={adminShops.data?.length || 0}>
                {(adminShops.data || []).map(shop => (
                  <div key={shop.id} className="flex items-center justify-between border-t border-[#e4d6c8] px-5 py-4">
                    <div className="min-w-0"><p className="truncate font-semibold">{shop.name}</p><p className="text-xs text-[#6c5b4f]">{shop.vendor?.displayName || 'Vendor'} · {shop.slug}</p></div>
                    <div className="flex items-center gap-2">
...
function SectionHeader({ title, aside }: { title: string; aside?: ReactNode }) {
  return <div className="flex flex-col gap-3 border-b border-[#e4d6c8] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
...
function Panel({ title, count, children }: { title: string; count: number; children: ReactNode }) {
  return <section className="overflow-hidden rounded-xl border border-[#e4d6c8] bg-white shadow-sm">
    <SectionHeader title={title} aside={<span className="rounded-full bg-[#f9f5f0] px-3 py-1 text-xs font-bold text-[#6c5b4f]">{count}</span>} />
    <div>{children || <p className="px-5 py-8 text-sm text-[#6c5b4f]">No records.</p>}</div>
  </section>
}
...
function Row({ title, meta, status }: { title: string; meta: string; status?: string }) {
  return <div className="min-w-0 px-5 py-3.5">
    ...
    <p className="mt-0.5 truncate text-xs text-[#6c5b4f]">{meta}</p>
...
function CaseRow({ title, meta, status, resolve, dismiss, onDelete }: { title: string; meta: string; status: string; resolve?: () => void; dismiss?: () => void; onDelete?: () => void }) {
  return <div className="flex flex-col gap-3 border-t border-[#e4d6c8] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
...
function PlanField({ label, value, onChange, render }: { label: string; value: string; onChange: (v: string) => void; render?: () => ReactNode }) {
  return <div>
    <label className="block text-xs font-semibold text-[#6c5b4f] mb-1">{label}</label>
    {render ? render() : <input value={value} onChange={e => onChange(e.target.value)} className="w-full rounded-lg border border-[#e4d6c8] px-3.5 py-2.5 text-sm outline-none transition focus:border-[#221b16]" />}
  </div>
}

function Btn({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return <button onClick={onClick} className="rounded-lg bg-[#221b16] px-3.5 py-1.5 text-xs font-bold text-[#f9f5f0] transition hover:bg-[#3a3028] active:scale-[0.97]">{children}</button>
}
