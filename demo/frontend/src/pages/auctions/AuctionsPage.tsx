...
export default function AuctionsPage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f9f5f0] text-[#221b16]">
      <section className="relative border-b border-[#e4d6c8] px-6 py-12">
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-5 font-[Fraunces] text-5xl leading-tight text-[#221b16] md:text-7xl">AtomDrops Live Auction Floor</h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[#6c5b4f] md:text-base">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-4">
            {tabs.map(item => (
              <button key={item.id} onClick={() => setTab(item.id)}
                className={`group rounded-2xl border p-4 text-left transition-all ${tab === item.id ? 'border-[#ee5a24] bg-[#ee5a24] text-white shadow-[0_20px_60px_rgba(238,90,36,0.25)]' : 'border-[#e4d6c8] bg-white text-[#221b16] hover:border-[#d7c7b8] hover:bg-[#f9f5f0]'}`}>
      ...
      <main className="mx-auto max-w-7xl px-6 py-10">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#ee5a24]">{tab === 'ALL' ? 'Full board' : tab.toLowerCase()}</p>
        ...
        {isLoading && visibleAuctions.length === 0 ? (
          <div className="rounded-3xl border border-[#e4d6c8] bg-white p-12 text-center text-[#6c5b4f]">Loading auction floor...</div>
        ) : visibleAuctions.length === 0 ? (
          <div className="rounded-3xl border border-[#e4d6c8] bg-white p-12 text-center">
            <p className="font-[Fraunces] text-2xl text-[#221b16]">No {tab.toLowerCase()} auctions right now.</p>
            <p className="mt-2 text-sm text-[#8c7564]">Check another board or come back when sellers publish new lots.</p>
          </div>
        ) : (
            ...
            {visibleAuctions.map(auction => {
              ...
              return (
                <Link key={auction.id} to={`/auctions/${auction.id}`}
                  className="group relative overflow-hidden rounded-[2rem] border border-[#e4d6c8] bg-white p-5 shadow-[0_24px_70px_rgba(34,27,22,0.08)] transition-all hover:-translate-y-1 hover:border-[#ee5a24]/60 hover:shadow-[0_28px_90px_rgba(238,90,36,0.15)]">
                  <div className="absolute right-0 top-0 h-44 w-44 rounded-bl-full bg-[#ee5a24]/5 blur-2xl transition group-hover:bg-[#ee5a24]/10" />
                  <div className="relative grid gap-5 md:grid-cols-[180px_1fr]">
                    <div className="space-y-3">
                      <div className="grid h-44 grid-cols-2 gap-2 overflow-hidden rounded-3xl bg-[#f9f5f0] p-2">
                        {images.length > 0 ? images.map((image, index) => (
                          ...
                          <div className="col-span-2 flex h-full items-center justify-center rounded-2xl border border-dashed border-[#d7c7b8] text-xs text-[#a28672]">No lot photos yet</div>
                        )}
                      </div>
                      <div className="rounded-2xl border border-[#e4d6c8] bg-[#f9f5f0] p-3">
                        <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-[0.18em] text-[#8c7564]"><span>Heat</span><span>{heat}%</span></div>
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full border border-[#e4d6c8] bg-[#f9f5f0] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-[#6c5b4f]">{auction.type}</span>
                      ...
                      <h3 className="mt-4 truncate font-[Fraunces] text-3xl text-[#221b16] group-hover:text-[#ee5a24]">{auction.title}</h3>
                      <p className="mt-1 text-sm text-[#6c5b4f]">Seller: {auction.vendor?.displayName || 'Verified seller'}</p>
                      ...
                      <div className="mt-5 rounded-2xl border border-[#e4d6c8] bg-[#f9f5f0] p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          <div className="min-w-0">
                            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[#8c7564]">Featured lot</p>
                            <p className="mt-1 truncate font-semibold text-[#221b16]">{featuredLot?.title || 'Waiting for lots'}</p>
                            {featuredLot?.conditionNote && <p className="mt-1 text-xs text-[#6c5b4f]">Condition: {featuredLot.conditionNote}</p>}
                          </div>
...
function Metric({ label, value, compact = false }: { label: string; value: number | string; compact?: boolean }) {
  return (
    <div className="rounded-2xl border border-[#e4d6c8] bg-white p-4 shadow-sm">
      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8c7564]">{label}</p>
      <p className={`mt-1 font-[Fraunces] text-[#221b16] ${compact ? 'text-xl' : 'text-3xl'}`}>{value}</p>
    </div>
  )
}
...
function BoardStat({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-2xl border border-[#e4d6c8] bg-[#f9f5f0] p-3">
      <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#8c7564]">{label}</p>
      <p className="mt-1 truncate text-sm font-bold text-[#221b16]">{value}</p>
    </div>
  )
}
