    ...
    <div className="min-h-screen bg-[#faf6f2]">
      ...
      <div className="mx-auto max-w-2xl px-4 sm:px-6 py-10 sm:py-16">
        <div className="animate-[fadeSlideUp_0.5s_ease-out] text-center">
          ...
          <h1 className="mt-6 font-[Fraunces] text-2xl sm:text-3xl font-semibold text-[#221b16] tracking-tight">
            Payment Successful!
          </h1>
          ...
          <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[#221b16] px-5 py-2">
            <span className="text-xs font-medium text-[#e4d6c8]">Order</span>
        ...
        <div className="mt-8 rounded-2xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] ring-1 ring-[#e4d6c8]/40">
          <div className="border-b border-[#e4d6c8]/40 px-5 py-4">
            <h2 className="font-semibold text-sm text-[#221b16]">Items Ordered ({itemCount})</h2>
          </div>
          <div className="divide-y divide-[#e4d6c8]/40">
            {order.items.map((item) => (
              <div key={item.id} className="flex items-center gap-3 px-5 py-3.5">
                <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-[#f9f5f0]">
                ...
                <p className="text-sm font-semibold text-[#221b16] shrink-0">৳{(item.unitPriceBdt * item.qty).toLocaleString('en-BD', { minimumFractionDigits: 2 })}</p>
              </div>
            ))}
          </div>
        ...
        <div className="mt-4 flex items-start gap-3 rounded-xl bg-amber-50 px-4 py-3.5 ring-1 ring-amber-200/60 animate-[fadeSlideUp_0.5s_ease-out_0.15s_both]">
          ...
          <div>
            <p className="text-xs font-semibold text-amber-700">Delivery Estimate</p>
            <p className="text-xs text-amber-600 mt-0.5">Expected within <strong>{deliveryEstimate}</strong></p>
          </div>
        </div>

        <div className="mt-8 flex flex-col sm:flex-row items-center gap-3 animate-[fadeSlideUp_0.5s_ease-out_0.25s_both]">
          <Link to={`/account/orders`}
            className="w-full sm:flex-1 inline-flex items-center justify-center gap-2 rounded-xl border border-[#e4d6c8] px-5 py-3 text-sm font-semibold text-[#221b16] hover:bg-[#f9f5f0] transition">
          ...
          <Link to="/products"
            className="w-full sm:flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-[#221b16] px-5 py-3 text-sm font-semibold text-[#faf6f2] hover:bg-[#3a3028] transition active:scale-[0.98]">
            Continue Shopping
