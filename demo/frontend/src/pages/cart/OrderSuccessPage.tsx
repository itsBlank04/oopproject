  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center">
        <svg className="h-8 w-8 animate-spin text-[#94A3B8]" viewBox="0 0 24 24" fill="none">
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="mx-auto max-w-2xl px-4 sm:px-6 py-10 sm:py-16">
        <div className="animate-[fadeSlideUp_0.5s_ease-out] text-center">
          ...
          <h1 className="mt-6 font-[Fraunces] text-2xl sm:text-3xl font-semibold text-[#1e293b] tracking-tight">
            Payment Successful!
          </h1>
          ...
          <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[#1e293b] px-5 py-2">
            <span className="text-xs font-medium text-[#cbd5e1]">Order</span>
            <span className="font-semibold text-[#f8fafc] tracking-wider">#{order.id}</span>
          </div>
        </div>

        <div className="mt-8 rounded-2xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] ring-1 ring-[#e0e7ff]/40">
          <div className="border-b border-[#e0e7ff]/40 px-5 py-4">
            <h2 className="font-semibold text-sm text-[#1e293b]">Items Ordered ({itemCount})</h2>
          </div>
          <div className="divide-y divide-[#e0e7ff]/40">
            {order.items.map((item) => (
              <div key={item.id} className="flex items-center gap-3 px-5 py-3.5">
                <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-[#eef2ff]">
                ...
                <p className="text-sm font-semibold text-[#1e293b] shrink-0">৳{(item.unitPriceBdt * item.qty).toLocaleString('en-BD', { minimumFractionDigits: 2 })}</p>
              </div>
            ))}
          </div>
          <div className="border-t border-[#e0e7ff]/40 px-5 py-4 space-y-1.5">
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center gap-3 animate-[fadeSlideUp_0.5s_ease-out_0.25s_both]">
          <Link to={`/account/orders`}
            className="w-full sm:flex-1 inline-flex items-center justify-center gap-2 rounded-xl border border-[#cbd5e1] px-5 py-3 text-sm font-semibold text-[#1e293b] hover:bg-[#f8fafc] transition">
          ...
          <Link to="/products"
            className="w-full sm:flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-[#1e293b] px-5 py-3 text-sm font-semibold text-[#f8fafc] hover:bg-[#334155] transition active:scale-[0.98]">
            Continue Shopping
