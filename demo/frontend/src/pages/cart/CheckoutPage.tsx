  ...
  const mainContent = items.length === 0 ? (
    <div className="mx-auto max-w-3xl px-6 py-20 text-center">
      <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#f9f5f0] mb-4">
        <svg className="h-7 w-7 text-[#e4d6c8]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
        </svg>
      </div>
      <p className="font-semibold text-[#221b16]">Your cart is empty</p>
    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
      <div className="anim-fade anim-fade-1">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="font-[Fraunces] text-xl sm:text-2xl font-semibold text-[#221b16] tracking-tight">Checkout</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr] anim-fade anim-fade-2">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="font-semibold text-[#221b16]">Shipping Address</h2>
          ...
          {addresses && addresses.length > 0 ? (
            <div className="mt-3 space-y-3">
              {addresses.map((addr) => (
                <label key={addr.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition-all ${selectedAddressId === addr.id ? 'border-[#221b16] bg-[#faf6f2] ring-1 ring-[#221b16]/10' : 'border-[#e4d6c8]/60 bg-white hover:border-[#e4d6c8]'}`}>
                  <input type="radio" name="address" checked={selectedAddressId === addr.id} onChange={() => setSelectedAddressId(addr.id)} className="mt-1 accent-[#221b16]" />
                  <div>
                    <p className="text-sm font-semibold text-[#221b16]">{addr.label} {addr.isDefault && <span className="text-[10px] text-[#8c7564] font-normal">(Default)</span>}</p>
          ...
          <button onClick={() => setShowAddressForm(!showAddressForm)} className="mt-3 text-sm font-semibold text-[#221b16] hover:underline">
        ...
        <div>
          <h2 className="font-semibold text-[#221b16]">Order Summary</h2>
          <div className="mt-3 space-y-3 rounded-2xl bg-white p-5 ring-1 ring-[#e4d6c8]/40 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            {items.map((item) => (
              <div key={item.id} className="flex items-center gap-3">
                ...
                <p className="text-sm font-semibold text-[#221b16] shrink-0">৳{(item.product.priceBdt * item.qty).toLocaleString('en-BD', { minimumFractionDigits: 2 })}</p>
              </div>
            ))}
            <div className="border-t border-[#e4d6c8]/40 pt-3 space-y-1">
              ...
              <div className="flex justify-between font-semibold text-[#221b16] pt-2 border-t border-[#e4d6c8]/40">
                <span>Total</span>
            ...
            {buttonHint && (
              <p className="mt-2 text-center text-xs text-[#8c7564]">{buttonHint}</p>
            )}
          </div>
        </div>
      </div>
      ...
      {step === 'choice' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl animate-[scaleIn_0.2s_ease-out]">
            <div className="px-6 py-5 text-center border-b border-[#e4d6c8]/40">
              ...
              <h2 className="font-semibold text-[#221b16] text-lg">Order placed!</h2>
              <p className="text-sm text-[#6c5b4f] mt-1">Order #{placedOrderId} has been placed successfully</p>
            </div>
            <div className="px-6 py-5 space-y-3">
              <p className="text-sm font-medium text-[#221b16]">Choose payment method</p>
              <button onClick={() => setStep('payment')}
                className="w-full flex items-center gap-4 rounded-xl border-2 border-[#221b16] bg-[#faf6f2] p-4 text-left transition-all hover:bg-[#f9f5f0]">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#221b16]">
                  <svg className="h-5 w-5 text-[#faf6f2]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                  </svg>
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[#221b16]">Pay Now</p>
                  <p className="text-xs text-[#8c7564]">Pay with bKash, Nagad, or Card</p>
                </div>
                <svg className="h-5 w-5 text-[#8c7564]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                </svg>
              </button>
              <button onClick={confirmCod} disabled={processingPayment}
                className="w-full flex items-center gap-4 rounded-xl border-2 border-[#e4d6c8]/60 bg-white p-4 text-left transition-all hover:border-[#e4d6c8] disabled:opacity-50">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#f9f5f0]">
                  <svg className="h-5 w-5 text-[#6c5b4f]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125" />
                  </svg>
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[#221b16]">Cash on Delivery</p>
                  <p className="text-xs text-[#8c7564]">Pay when you receive your order</p>
                </div>
                {processingPayment ? (
                  <svg className="h-5 w-5 animate-spin text-[#8c7564]" viewBox="0 0 24 24" fill="none">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => { setStep('choice'); setPaymentMethod(null) }}>
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl animate-[scaleIn_0.2s_ease-out]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-[#e4d6c8]/40 px-6 py-4">
              ...
              <button onClick={() => { setStep('choice'); setPaymentMethod(null) }} className="text-[#8c7564] hover:text-[#221b16]">
            ...
            <div className="flex items-center gap-3 border-t border-[#e4d6c8]/40 px-6 py-4">
              <button onClick={() => { setStep('choice'); setPaymentMethod(null) }}
                className="flex-1 rounded-xl border border-[#e4d6c8] px-4 py-2.5 text-sm font-medium text-[#6c5b4f] hover:bg-[#f9f5f0] transition-colors">
                Back
              </button>
