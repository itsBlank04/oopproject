...
export default function VendorProductsPage() {
  ...
  if (!user) {
    return <div className="mx-auto max-w-4xl px-6 py-20 text-center"><p className="text-[#64748b]">Sign in as a vendor</p><Link to="/auth/login" className="mt-4 inline-block rounded-full bg-[#1e293b] px-6 py-3 text-sm font-semibold text-[#f8fafc]">Sign in</Link></div>
  }

  if (!hasRole('VENDOR')) {
    return (
      <div className="mx-auto max-w-4xl px-6 py-20 text-center">
        <div className="mx-auto max-w-xl rounded-3xl border border-[#e0e7ff] bg-white p-8 shadow-sm">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#eef2ff]">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-7 w-7 text-[#1e293b]">
              <path d="M13.5 21v-7.5a.75.75 0 0 1 .75-.75h3a.75.75 0 0 1 .75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349M3.75 21V9.349m0 0a3.001 3.001 0 0 0 3.75-.615A2.993 2.993 0 0 0 9.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 0 0 2.25 1.016c.896 0 1.7-.393 2.25-1.015a3.001 3.001 0 0 0 3.75.614m-16.5 0a3.004 3.004 0 0 1-.621-4.72l1.189-1.19A1.5 1.5 0 0 1 5.378 3h13.243a1.5 1.5 0 0 1 1.06.44l1.19 1.189a3 3 0 0 1-.621 4.72M6.75 18h3.75a.75.75 0 0 0 .75-.75V13.5a.75.75 0 0 0-.75-.75H6.75a.75.75 0 0 0-.75.75v3.75c0 .414.336.75.75.75Z" />
            </svg>
          </div>
          <p className="mt-4 font-semibold text-[#1e293b]">Upgrade to Merchant to access products</p>
          <p className="mt-2 text-sm text-[#94A3B8]">List products, manage inventory, and sell with your own shop page.</p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-xs text-[#64748b]">
            <span className="rounded-full border border-[#e0e7ff] bg-[#f8fafc] px-3 py-1">One-time 99 TK</span>
            <span className="rounded-full border border-[#e0e7ff] bg-[#f8fafc] px-3 py-1">Instant activation</span>
          </div>
          <Link to="/profile#role-upgrade" className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#1e293b] px-6 py-3 text-sm font-semibold text-[#f8fafc]">
            Upgrade in Profile
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      {/* Header */}
      <div className="border-b border-[#e0e7ff] bg-white">
        <div className="mx-auto max-w-6xl px-6 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="font-[Fraunces] text-3xl tracking-tight text-[#1e293b]">Products</h1>
              <p className="mt-1 text-sm text-[#94A3B8]">Manage your inventory, stock levels, and listings</p>
            </div>
            
            <div className="flex flex-wrap items-center gap-2">
              {/* Shop filter */}
              {shops.length > 0 && (
                <div className="flex items-center gap-2 bg-[#f8fafc] border border-[#e0e7ff] px-3 py-2 rounded-xl shadow-sm text-xs">
                  <span className="font-semibold text-[#94A3B8]">Shop:</span>
                  <select
                    value={selectedShopId || ''}
                    onChange={(e) => setSelectedShopId(e.target.value ? Number(e.target.value) : null)}
                    className="font-bold text-[#1e293b] bg-transparent focus:outline-none cursor-pointer"
                  >
              ...
              <button onClick={() => { resetForm(); setShowForm(!showForm) }}
                className="flex items-center gap-2 rounded-xl bg-[#1e293b] px-5 py-2.5 text-sm font-semibold text-[#f8fafc] transition hover:bg-[#4338ca] active:scale-[0.97]">
          ...
          {stats && (
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { label: 'Total Products', value: stats.total, color: 'text-[#1e293b]', bg: 'bg-[#f8fafc]' },
                ...
                { label: 'Out of Stock', value: stats.outOfStock, color: stats.outOfStock > 0 ? 'text-red-600' : 'text-[#94A3B8]', bg: stats.outOfStock > 0 ? 'bg-red-50' : 'bg-[#f8fafc]' },
              ].map(s => (
                <div key={s.label} className={`rounded-xl ${s.bg} px-4 py-3`}>
                  <p className={`text-2xl font-bold ${s.color}`}>{s.value}</p>
                  <p className="mt-0.5 text-xs text-[#94A3B8]">{s.label}</p>
                </div>
              ))}
            </div>
          )}
      ...
      {(showForm || editProductId) && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-in-from-right">
            <div className="sticky top-0 z-10 border-b border-[#e0e7ff] bg-white px-6 py-4 flex items-center justify-between">
              <h2 className="font-[Fraunces] text-xl text-[#1e293b]">{editProductId ? 'Edit Product' : 'New Product'}</h2>
              <button onClick={resetForm} className="flex h-8 w-8 items-center justify-center rounded-full text-[#94A3B8] hover:bg-[#f8fafc] transition">
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              <div>
                <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Target Shop / Outlet</label>
                <select 
                  value={form.shopId} 
                  onChange={e => setForm({ ...form, shopId: e.target.value })}
                  disabled={!!editProductId}
                  className="mt-1.5 w-full rounded-xl border border-[#cbd5e1] bg-[#f8fafc] px-4 py-2.5 text-sm text-[#1e293b] outline-none transition focus:border-[#1e293b] focus:bg-white focus:ring-1 focus:ring-[#1e293b]/10 disabled:opacity-50"
                >
              ...
              <div>
                <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Product Name</label>
                <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                  placeholder="e.g. Wireless Bluetooth Headphones"
                  className="mt-1.5 w-full rounded-xl border border-[#cbd5e1] bg-[#f8fafc] px-4 py-2.5 text-sm text-[#1e293b] outline-none transition focus:border-[#1e293b] focus:bg-white focus:ring-1 focus:ring-[#1e293b]/10" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Price (৳)</label>
                  <input type="number" step="0.01" value={form.priceBdt} onChange={e => setForm({ ...form, priceBdt: e.target.value })}
                    placeholder="0.00"
                    className="mt-1.5 w-full rounded-xl border border-[#cbd5e1] bg-[#f8fafc] px-4 py-2.5 text-sm text-[#1e293b] outline-none transition focus:border-[#1e293b] focus:bg-white focus:ring-1 focus:ring-[#1e293b]/10" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Category</label>
              ...
              <div>
                <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Photos</label>
              ...
              <button onClick={() => editProductId ? updateProduct.mutate() : createProduct.mutate()}
                ...
                className="w-full rounded-xl bg-[#1e293b] py-3 text-sm font-semibold text-[#f8fafc] transition hover:bg-[#4338ca] disabled:opacity-40 active:scale-[0.98]">
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        {products.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-[#e0e7ff] p-16 text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#f8fafc]">
              <svg className="h-8 w-8 text-[#cbd5e1]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0l-3-3m3 3l3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
            </div>
            <p className="mt-4 font-semibold text-[#1e293b]">No products yet</p>
            <p className="mt-1 text-sm text-[#94A3B8]">Click "Add Product" to create your first listing</p>
            {productsFetching && (
              <p className="mt-2 text-xs text-[#cbd5e1]">Checking your catalog...</p>
            )}
          </div>
        ) : (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {products.map(p => {
              ...
              return (
                <div key={p.id} className="group relative rounded-2xl bg-white shadow-sm ring-1 ring-[#e0e7ff]/60 transition-all hover:shadow-md hover:ring-[#cbd5e1]">
                  {/* Status badge */}
                  ...
                  <button type="button" onClick={() => setLightbox({
                    ...
                    className="aspect-[4/3] w-full overflow-hidden rounded-t-2xl bg-[#f8fafc]"
                  >
                    {p.images?.[0] ? (
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <h3 className="truncate font-semibold text-[#1e293b]">{p.name}</h3>
                        <p className="mt-0.5 text-xs text-[#94A3B8]">{p.category?.name}</p>
                      </div>
                      <p className="shrink-0 font-bold text-[#1e293b]">৳{p.priceBdt.toLocaleString('en-BD')}</p>
                    </div>
                    ...
                    <div className="mt-4">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-[#94A3B8]">Low: {s.lowStockThreshold}</span>
                      </div>
                    ...
                    <div className="mt-4 flex items-center gap-2">
                      <button onClick={() => setStock(p.id, Math.max(0, s.stockQty - 1), s.lowStockThreshold)}
                        className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-[#cbd5e1] text-[#64748b] transition hover:bg-[#f8fafc] active:scale-90">
                      ...
                      <input
                        ...
                        className="w-16 rounded-lg border border-[#cbd5e1] bg-[#f8fafc] px-2 py-1.5 text-center text-xs font-semibold text-[#1e293b] outline-none focus:border-[#1e293b] [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                      />
                      ...
                      <div className="ml-auto flex items-center gap-1">
                        <label className="text-[10px] text-[#94A3B8]">Alert:</label>
                        <input
                          type="number" min="0"
                          value={s.lowStockThreshold}
                          onChange={e => setStock(p.id, s.stockQty, Math.max(0, parseInt(e.target.value) || 0))}
                          className="w-12 rounded-lg border border-[#cbd5e1] bg-[#f8fafc] px-1.5 py-1 text-center text-[10px] text-[#1e293b] outline-none focus:border-[#1e293b] [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                        />
                      </div>
                    </div>

                    {/* Shipping toggle */}
                    <div className="mt-4 border-t border-[#eef2ff] pt-3">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-[#64748b]">Shipping</span>
                        <button
                          ...
                          className={`relative inline-flex h-6 w-10 cursor-pointer items-center rounded-full transition-all ${
                            p.shippingType === 'PAID' ? 'bg-[#1e293b]' : 'bg-[#cbd5e1]'
                          }`}
                        >
                    ...
                    <div className="mt-4 flex gap-2">
                      <button onClick={() => startEdit(p)}
                        className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-[#cbd5e1] py-2 text-xs font-semibold text-[#1e293b] transition hover:bg-[#f8fafc] active:scale-[0.97]">
