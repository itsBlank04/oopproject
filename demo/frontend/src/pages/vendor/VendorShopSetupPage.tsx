      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-[Fraunces] text-2xl font-bold text-[#1a1512]">Shop Setup</h1>
              ...
              {steps.map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition ${
                    ...
                    i === step ? 'bg-[#1a1512] text-white' :
                    'bg-[#e4d6c8] text-[#8c7564]'
                  }`}>
                    {i < step ? <Check className="h-3.5 w-3.5" /> : i + 1}
                  </div>
                  <span className={`text-xs font-semibold ${i === step ? 'text-[#1a1512]' : 'text-[#8c7564]'}`}>{s}</span>
            ...
            <form onSubmit={step === 2 ? handleSubmit : (e) => e.preventDefault()}>
                ...
                <div className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" required placeholder="e.g. ATOM Gadget Spot"
                      ...
                      className="w-full rounded-xl border border-[#d7c7b8] bg-white px-4 py-2.5 text-sm text-[#1a1512] outline-none transition focus:border-[#1a1512]" />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea rows={3} placeholder="What does your shop sell?"
                      ...
                      className="w-full resize-none rounded-xl border border-[#d7c7b8] bg-white px-4 py-2.5 text-sm text-[#1a1512] outline-none transition focus:border-[#1a1512]" />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-3">
                      <input type="url" placeholder="https://example.com/logo.jpg"
                        ...
                        className="flex-1 rounded-xl border border-[#d7c7b8] bg-white px-4 py-2.5 text-sm text-[#1a1512] outline-none transition focus:border-[#1a1512]" />
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-dashed border-[#d7c7b8] bg-[#f9f5f0]">
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div className="rounded-xl border border-[#e4d6c8]/60 bg-white p-5">
                    <h3 className="text-sm font-semibold text-[#1a1512] mb-4">Review Your Shop</h3>
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between"><span className="text-[#8c7564]">Name</span><span className="font-medium text-[#1a1512]">{name}</span></div>
                      <div className="flex justify-between"><span className="text-[#8c7564]">Category</span><span className="font-medium text-[#1a1512]">{categories.find(c => c.id === Number(category))?.name || 'None'}</span></div>
                      <div className="flex justify-between"><span className="text-[#8c7564]">Location</span><span className="font-medium text-[#1a1512]">{location || 'Not set'}</span></div>
              ...
              <div className="mt-8 flex items-center justify-between">
                <button type="button" onClick={() => setStep(Math.max(0, step - 1))}
                  disabled={step === 0}
                  className="flex items-center gap-1.5 rounded-xl border border-[#d7c7b8] bg-white px-5 py-2.5 text-xs font-semibold text-[#6c5b4f] hover:bg-[#f9f5f0] transition disabled:opacity-40">
                  <ArrowLeft className="h-4 w-4" /> Back
                </button>
                {step < 2 ? (
                  <button type="button" onClick={() => setStep(step + 1)}
                    className="flex items-center gap-1.5 rounded-xl bg-[#1a1512] px-5 py-2.5 text-xs font-semibold text-white hover:bg-[#3a3028] transition">
                    Next <ArrowRight className="h-4 w-4" />
                  </button>
                ) : (
                  <button type="submit" disabled={createShopMutation.isPending}
                    className="flex items-center gap-1.5 rounded-xl bg-[#1a1512] px-6 py-2.5 text-xs font-semibold text-white hover:bg-[#3a3028] transition disabled:opacity-50">
          ...
          <div className="hidden lg:block flex-1">
            <div className="sticky top-8">
              ...
              <div className="overflow-hidden rounded-2xl border border-[#e4d6c8]/60 bg-white shadow-sm">
                {/* Banner preview */}
                <div className="h-32 bg-gradient-to-r from-[#e4d6c8] to-[#f1e9e0]">
                ...
                <div className="px-5 pb-5">
                  ...
                  <h3 className="font-[Fraunces] text-lg font-bold text-[#1a1512]">{name || 'Your Shop Name'}</h3>
