    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] py-12 px-6">
        <div className="mx-auto max-w-5xl animate-pulse space-y-8">
          <div className="h-40 rounded-2xl bg-[#e0e7ff]/30" />
          <div className="grid gap-6 md:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-96 rounded-xl bg-[#e0e7ff]/30" />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#f8fafc] py-12 px-6">
      <div className="mx-auto max-w-5xl">
        <div className="mb-10 text-center md:text-left">
          <h1 className="font-[Fraunces] text-4xl font-bold text-[#1e293b]">Vendor Subscriptions</h1>
          <p className="text-[#94A3B8] text-sm mt-2">Manage your storefront slots, staff accounts, and premium feature allocations.</p>
        </div>

        {/* Current Subscription Status */}
        {summary && (
          <div className="mb-12 rounded-2xl border border-[#e0e7ff]/60 bg-white p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-[#94A3B8]">Current Active Plan</span>
              ...
              <h2 className="font-[Fraunces] text-2xl font-bold text-[#1e293b]">
                {summary.planDisplayName}
              </h2>
              <div className="text-sm text-[#94A3B8] flex flex-wrap gap-x-4 gap-y-1">
            ...
            <div className="flex items-center gap-4 bg-[#f8fafc] border border-[#e0e7ff]/50 p-4 rounded-xl w-full md:w-auto">
              <div className="text-center w-full">
                <p className="text-[10px] uppercase font-semibold tracking-wider text-[#94A3B8]">Shop Capacity</p>
                <div className="flex items-baseline justify-center gap-1 mt-1">
                  <span className="text-2xl font-bold text-[#1e293b]">{summary.currentShopCount}</span>
                  <span className="text-sm text-[#94A3B8]">/ {summary.maxShops === -1 ? '∞' : summary.maxShops}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {deals.length > 0 && (
          <div className="mb-12 space-y-4">
            {(deals).filter(d => new Date(d.endsAt).getTime() > Date.now()).map(d => (
              <div key={d.id} className="rounded-2xl bg-gradient-to-r from-[#94A3B8] to-[#64748b] p-6 text-white shadow-md relative overflow-hidden">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-white border border-[#e0e7ff] p-1 rounded-xl flex gap-1 shadow-sm">
            <button
              onClick={() => setBillingCycle('MONTHLY')}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                billingCycle === 'MONTHLY' ? 'bg-[#94A3B8] text-white' : 'text-[#94A3B8] hover:bg-[#94A3B8]/5'
              }`}
            >
              Monthly Billing
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                billingCycle === 'YEARLY' ? 'bg-[#94A3B8] text-white' : 'text-[#94A3B8] hover:bg-[#94A3B8]/5'
              }`}
            >
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl border bg-white p-6 shadow-sm flex flex-col justify-between transition-all duration-300 hover:shadow-md ${
                  isCurrent ? 'border-2 border-[#94A3B8]' : 'border-[#e0e7ff]/60'
                }`}
              >
                {isCurrent && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#94A3B8] px-3 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">
                    Current Plan
                  </span>
                )}
                <div>
                  <div className="mb-4">
                    <div className="flex items-center justify-between">
                      <h3 className="font-[Fraunces] text-xl font-bold text-[#1e293b]">{plan.displayName}</h3>
                  ...
                  <div className="mb-3 flex items-baseline gap-1">
                    <span className="font-[Fraunces] text-3xl font-bold text-[#1e293b]">{displayPrice}</span>
                    {price > 0 && (
        ...
        {selectedPlan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="w-full max-w-md rounded-2xl border border-[#e0e7ff] bg-white p-6 shadow-xl relative animate-in fade-in zoom-in-95 duration-200">
              ...
              {checkoutStep !== 'success' ? (
                <form onSubmit={handleCheckoutSubmit} className="space-y-6">
                  <div>
                    <h3 className="font-[Fraunces] text-2xl text-[#1e293b] font-bold">Secure Checkout</h3>
                    <p className="text-[#94A3B8] text-xs mt-1">
                      Upgrading to <span className="font-semibold text-[#1e293b]">{selectedPlan.displayName}</span> ({billingCycle.toLowerCase()})
                    </p>
                  </div>
                  ...
                  {checkoutStep === 'details' ? (
                    <div className="space-y-4">
                      {/* Summary fields */}
                      <div className="rounded-xl bg-[#f8fafc] border border-[#e0e7ff]/40 p-4 space-y-2">
                        ...
                        <div className="border-t border-[#e0e7ff]/40 pt-2 flex justify-between text-sm font-bold text-[#1e293b]">
                      ...
                      <button
                        type="submit"
                        className="w-full py-3 bg-[#94A3B8] text-white rounded-xl text-sm font-semibold hover:bg-[#64748b] transition-all"
                      >
                        Proceed to Payment
                      </button>
                    </div>
                    ...
                    <div className="space-y-4">
                      {/* Payment Method Tabs */}
                      <div className="grid grid-cols-3 gap-2 bg-gray-50 border border-[#e0e7ff] p-1 rounded-xl">
                        <button
                          type="button"
                          onClick={() => { setPaymentMethod('bkash'); setErrorMsg(''); }}
                          className={`py-2 text-xs font-bold rounded-lg transition-all ${
                            paymentMethod === 'bkash' ? 'bg-[#e2125f] text-white' : 'text-[#94A3B8] hover:bg-gray-100'
                          }`}
                        >
                          bKash
                        </button>
                        <button
                          ...
                          className={`py-2 text-xs font-bold rounded-lg transition-all ${
                            paymentMethod === 'nagad' ? 'bg-[#4F46E5] text-white' : 'text-[#94A3B8] hover:bg-gray-100'
                          }`}
                        >
                          Nagad
                        </button>
                        <button
                          ...
                          className={`py-2 text-xs font-bold rounded-lg transition-all ${
                            paymentMethod === 'card' ? 'bg-slate-700 text-white' : 'text-[#94A3B8] hover:bg-gray-100'
                          }`}
                        >
                          Card
                        </button>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full border border-gray-300 rounded-xl py-2 pl-14 pr-4 text-sm focus:outline-none focus:border-[#94A3B8]"
                            />
                          </div>

                          <label className="block text-xs font-semibold text-[#94A3B8] mt-3">Account PIN</label>
                          <input
                            type="password"
                            required
                            placeholder="XXXX"
                            ...
                            className="w-full border border-gray-300 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-[#94A3B8]"
                          />
                        </div>
                      ) : (
                        <div className="space-y-3">
                          <label className="block text-xs font-semibold text-[#94A3B8]">Credit/Debit Card Number</label>
                          <input
                            type="text"
                            required
                          ...
                          <label className="block text-xs font-semibold text-[#94A3B8] mt-3">CVV / Security Code</label>
                          <input
                            type="password"
                            required
                            placeholder="***"
                ...
                <div className="text-center py-6 space-y-4">
                  ...
                  <div>
                    <h3 className="font-[Fraunces] text-2xl text-[#1e293b] font-bold">Subscription Upgraded!</h3>
                    <p className="text-[#94A3B8] text-xs mt-1">
                      You are now subscribed to the <span className="font-semibold text-[#1e293b]">{selectedPlan?.displayName}</span> package.
                    </p>
                  </div>
                  <button
                    onClick={() => setSelectedPlan(null)}
                    className="w-full py-2.5 bg-[#94A3B8] text-white rounded-xl text-xs font-semibold hover:bg-[#64748b] transition-all"
                  >
                    Return to Dashboard
                  </button>
                </div>
