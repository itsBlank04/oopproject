      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type="button" disabled={!interactive}
          ...
          className={`text-lg ${interactive ? 'cursor-pointer hover:scale-110' : 'cursor-default'} transition ${
            star <= value ? 'text-amber-500' : 'text-[#cbd5e1]'
          }`}
        >
          {star <= value ? '\u2605' : '\u2606'}
        </button>
...
function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    <div className="rounded-xl border border-[#e0e7ff] bg-white p-4">
      <div className="flex items-center gap-3">
        <div className="h-9 w-9 overflow-hidden rounded-full bg-[#e0e7ff]">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-[#1e293b]">{review.reviewer?.displayName}</p>
      ...
      {review.comment && (
        <p className="mt-2 text-sm leading-relaxed text-[#475569]">{review.comment}</p>
      )}
    </div>
  )
}
...
export default function ProductDetailPage() {
    ...
    return (
      <div className="mx-auto max-w-5xl px-6 py-10">
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="h-96 animate-pulse rounded-2xl bg-[#e0e7ff]" />
          <div className="space-y-4">
            <div className="h-4 w-32 animate-pulse rounded bg-[#e0e7ff]" />
            <div className="h-10 w-64 animate-pulse rounded bg-[#e0e7ff]" />
            <div className="h-8 w-40 animate-pulse rounded bg-[#e0e7ff]" />
          </div>
        </div>
      </div>
    )
  ...
  if (error || !product) {
    return (
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        <Link to="/products" className="mt-4 inline-block text-sm font-semibold text-[#1e293b] underline">
          Back to products
        </Link>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="mx-auto max-w-5xl px-6 py-10">
        <Link to="/products" className="text-sm text-[#64748b] hover:underline">&larr; Back</Link>
        <div className="mt-6 grid gap-10 lg:grid-cols-2">
          {/* Image gallery */}
          <div>
            <div
              className="group relative aspect-square overflow-hidden rounded-2xl bg-[#eef2ff]"
              tabIndex={0}
              onKeyDown={(e) => {
          ...
          <div>
            ...
            <h1 className="mt-2 font-[Fraunces] text-4xl text-[#1e293b]">{product.name}</h1>
            <p className="mt-4 font-[Fraunces] text-3xl text-[#1e293b]">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              {images.length > 0 && (
                <span className="text-xs text-[#94A3B8]">{images.length} photo{images.length > 1 ? 's' : ''}</span>
              )}
              {inStock ? (
                lowStock ? (
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                ...
                className="rounded-xl border border-[#cbd5e1] px-4 py-3 text-lg hover:bg-[#eef2ff] transition"
              >
                ♡
              </button>
            </div>
            ...
            {vendorProfile && (
              <div className="mt-6 rounded-2xl border border-[#e0e7ff] bg-white/80 p-5 shadow-sm backdrop-blur-sm">
                <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.15em] text-[#94A3B8]">Meet the Maker</p>
                <div className="flex items-center gap-4">
                  <Link to={`/shop/${vendorProfile.shopSlug}`} className="h-14 w-14 flex-shrink-0 overflow-hidden rounded-full bg-[#e0e7ff] ring-2 ring-[#eef2ff]">
                    {(vendorProfile.logoUrl || vendorProfile.avatarUrl) ? (
                      ...
                      <div className="flex h-full items-center justify-center font-[Fraunces] text-xl text-[#64748b]">
                        {vendorProfile.shopName?.charAt(0) || vendorProfile.displayName?.charAt(0)}
                      </div>
                    )}
                  </Link>
                  <div className="flex-1 min-w-0">
                    <Link to={`/shop/${vendorProfile.shopSlug}`} className="flex items-center gap-2 font-semibold text-[#1e293b] hover:underline">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-[#94A3B8]">
                    ...
                    {vendorProfile.location && (
                      <p className="mt-0.5 text-xs text-[#94A3B8]">{vendorProfile.location}</p>
                    )}
                  </div>
                </div>
                {vendorProfile.bio && (
                  <p className="mt-3 border-t border-[#eef2ff] pt-3 text-xs leading-relaxed text-[#64748b]">
                    {vendorProfile.bio}
                  </p>
                )}
                <div className="mt-4 flex gap-2">
                  <Link to={`/shop/${vendorProfile.shopSlug}`}
                    className="flex-1 rounded-xl bg-[#1e293b] py-2.5 text-center text-xs font-semibold text-[#f8fafc] hover:bg-[#4338ca] transition">
                    View Storefront
                  </Link>
                  <button onClick={() => { setReportType('VENDOR'); setShowReportModal(true) }}
                    className="rounded-xl border border-[#cbd5e1] px-4 py-2.5 text-xs font-semibold text-[#64748b] hover:bg-[#f8fafc] transition">
                    Report
                  </button>
                </div>
              </div>
        ...
        {/* Reviews section */}
        <div className="mt-12">
          <h2 className="font-[Fraunces] text-2xl text-[#1e293b]">
            Reviews ({reviews.length})
          </h2>
          {/* Review form */}
          {user ? (
            isOwner ? (
              <div className="mt-4 rounded-xl border border-[#e0e7ff] bg-[#f8fafc] p-4 text-center text-sm text-[#94A3B8]">
                You cannot review your own product
              </div>
            ) : (
              <div className="mt-4 rounded-xl border border-[#e0e7ff] bg-white p-4">
                <p className="text-sm font-semibold text-[#1e293b]">Write a review</p>
                <div className="mt-2">
                ...
                <textarea value={reviewComment} onChange={(e) => setReviewComment(e.target.value)}
                  ...
                  rows={3}
                  className="mt-3 w-full resize-none rounded-xl border border-[#cbd5e1] bg-[#f8fafc] px-4 py-3 text-sm text-[#1e293b] outline-none transition focus:border-[#1e293b]" />
                <div className="mt-3 flex justify-end">
                  <button onClick={handleSubmitReview} disabled={submitting}
                    className="rounded-xl bg-[#1e293b] px-6 py-2.5 text-sm font-semibold text-[#f8fafc] hover:bg-[#4338ca] transition disabled:opacity-50">
            ...
            <div className="mt-4 rounded-xl border border-[#e0e7ff] bg-white p-4 text-center text-sm text-[#94A3B8]">
              <Link to="/auth/login" className="font-semibold text-[#1e293b] underline">Sign in</Link> to leave a review
            </div>
          )}
          {/* Reviews list */}
          <div className="mt-6 space-y-3">
            {reviews.length === 0 ? (
              <p className="text-sm text-[#94A3B8]">No reviews yet. Be the first!</p>
            ) : (
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm"
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-[Fraunces] text-lg text-[#1e293b]">
              Report {reportType === 'PRODUCT' ? 'Product' : 'Vendor'}
            </h3>
            <textarea value={reportReason} onChange={(e) => setReportReason(e.target.value)}
              placeholder="Tell us why you're reporting this..."
              rows={4}
              className="mt-4 w-full resize-none rounded-xl border border-[#cbd5e1] bg-[#f8fafc] px-4 py-3 text-sm text-[#1e293b] outline-none transition focus:border-[#1e293b]" />
            <div className="mt-4 flex justify-end gap-3">
              <button onClick={() => setShowReportModal(false)}
                className="rounded-xl border border-[#cbd5e1] px-5 py-2.5 text-sm font-semibold text-[#64748b] hover:bg-[#eef2ff] transition">
                Cancel
              </button>
